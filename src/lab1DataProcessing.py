...
if len(col_sortata) % 2 == 0:
    ...
    print("Mediana primei coloane: %f" % (
            col_sortata[int(npArray.shape[0] / 2)] + col_sortata[int(npArray.shape[0] / 2) + 1]) / 2)
...
a = np.dot(npArray, ponderi)

# In[65]:
print(npArray.shape)
npArray = np.append(npArray, values=np.dot(npArray, ponderi).reshape(150, 1), axis=1)
